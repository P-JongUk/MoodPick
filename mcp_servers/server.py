...
def _is_playable(status: dict) -> bool:
    """YouTube videos.list status 필드 기반으로 앱 내 재생 가능 여부를 판단한다.

    uploadStatus가 "processed"이고, privacyStatus가 public/unlisted이며,
    embeddable이 명시적으로 True인 경우에만 True를 반환한다.
    embeddable이 None(필드 없음)인 경우도 보수적으로 제외한다.
    """
...
async def search_youtube(
    ...
    if allow_shorts:
        ...
        try:
            ...
            status_resp = await asyncio.to_thread(status_req.execute)
            t_status = time.perf_counter() - _t
        except (HttpError, Exception) as exc:
            logger.warning("videos.list(status) failed for allow_shorts path: %s", exc)
            t_status = None
            status_resp = {}
    ...
    logger.info(
        "YouTube search complete query=%r candidates=%s filtered=%s kept=%s allow_shorts=%s",
        query,
        len(candidates),
        filtered_out,
        len(final_results),
