...
> 빠른 인수인계/컨텍스트 복구용 요약은 [`CONTEXT.md`](./CONTEXT.md)를 먼저 확인하세요.
...
### 완료된 항목 (AI 제외)
- [x] 프론트엔드 로그인/회원가입(이메일) UI 및 Supabase Auth 연동
...
**중요 원칙**
- 5월 13일까지는 "기획한 핵심 플로우가 끊김 없이 동작하는 프로토타입" 완성을 최우선으로 합니다.
- 베타 테스트는 5/13 이후로 분리하여 진행하고, 수집된 이슈는 우선순위 백로그로 관리합니다.
- 일정 리스크가 생기면 기능 추가보다 안정화와 오류 제거를 우선합니다.
...
**Phase 1 완료 체크**:
- [ ] Vite + React + TypeScript 환경 완성
- [x] Tailwind CSS 설정 완료
- [ ] Context API로 전역 상태 관리 시스템 구축
- [x] 모든 주요 페이지 UI 구현
- [ ] 개발 서버에서 오류 없이 실행됨

#### Phase 1 진행 메모 (2026-04-04)
- 현재 프론트는 Vite가 아닌 Next.js 기반으로 운영 중
- 대시보드/로그인/문진/피드백 UI 및 Supabase 연동 코드가 1차 반영됨
- 인증 상태는 컴포넌트 로컬 상태 기반이며 Context 전환은 미완료
- 개발 서버 실행 안정성 확인은 미완료
...
**Phase 2 완료 체크**:
- [ ] FastAPI 프로젝트 구조 완성
- [ ] 모든 주요 라우트 구현 (인증, 감정, 상담, 사용자)
- [ ] Supabase 연결 완료
- [ ] Swagger UI에서 모든 엔드포인트 확인 가능
- [ ] 개발 서버에서 오류 없이 실행됨

#### Phase 2 진행 메모 (2026-04-04)
- backend/app 최소 FastAPI 골격을 추가함
- 남은 작업은 Python 패키지 설치, 실제 Supabase 연결값 반영, 라우트별 데이터 저장 로직 구현
- 서버 실행 후 /health, /auth/*, /emotion/*, /counseling/* 동작 검증이 필요함
...
- [ ] **4-1-1. Supabase 대시보드에서 소셜 로그인 설정**
  - Google OAuth 활성화
  - GitHub OAuth 활성화
  - 카카오 로그인은 백엔드 연결 이후 별도 단계에서 적용

- [ ] **4-1-2. 리다이렉트 URL 설정**
  - `http://localhost:5173/callback`

- [ ] **4-1-2-1. 인증 적용 순서 정리**
  - 1차: 이메일/비밀번호 + Google OAuth
  - 2차: 백엔드 연결 후 카카오 OAuth 추가
...
#### 4-11. 프론트 하드코딩/더미 데이터 실연동 전환
- [ ] **4-11-1. 하드코딩/더미 데이터 목록화**
  - 화면별(홈/상담/기록/마이페이지)로 하드코딩된 데이터 소스 정리
  - UI Mock 데이터와 실제 저장 대상 테이블 매핑 문서화

- [ ] **4-11-2. 하드코딩 상수 → 서비스 레이어 치환**
  - 컴포넌트 내부 상수를 `services/*` 호출 결과로 교체
  - 로딩/빈 상태/에러 상태를 포함한 UI 상태 분기 추가

- [ ] **4-11-3. 더미 기록 화면 API 연동**
  - 감정 기록/상담 이력/시청 기록을 실제 API 응답으로 렌더링
  - API 스키마 불일치 시 타입(`types/*`)과 매퍼 함수 정리

- [ ] **4-11-4. 임시 fallback 제거 기준 확정**
  - 개발 편의용 fallback 데이터는 feature flag로 제한
  - QA 통과 후 프로덕션 빌드에서 fallback 비활성화 확인
...
- 전역 AuthProvider 추가로 세션 상태를 앱 전체에서 읽을 수 있게 정리함
...
**목표**: 5/13까지 완성 프로토타입 배포 가능 상태 달성  
**소요 시간**: 마감 전 5-7일 집중 구간  
**선행 조건**: Phase 4 완료
...
#### 5-11. 베타 테스트 준비 (5/13 이후)
- [ ] **5-11-1. 내부/협력사 대상 베타 시나리오 작성**

- [ ] **5-11-2. 피드백 수집 폼 및 우선순위 기준 정의**

- [ ] **5-11-3. 베타 개선 백로그 운영 시작**

**Phase 5 완료 체크**:
- [ ] 핵심 사용자 플로우(로그인→상담→추천→피드백→기록)가 배포 환경에서 동작
- [ ] 치명 오류(P0/P1) 0건
- [ ] 주요 화면 모바일/데스크톱 사용 가능
- [ ] 5/13 제출 가능한 프로토타입 문서와 데모 링크 준비 완료
...
### 5월 13일 프로토타입 완성 기준 (필수)
- [ ] 로그인/세션 시작/문진/추천/피드백/기록 조회까지 한 번에 동작
- [ ] Supabase 실데이터 저장/조회 확인
- [ ] 핵심 페이지 모바일/데스크톱 반응형 확인
- [ ] 치명적 오류(P0/P1) 제거
- [ ] 제출용 데모 시나리오 및 README 최신화
...
### 5/13 이후 베타 테스트 기준 (선택)
- [ ] 협력사/학생 대상 베타 모집 및 가이드 배포
- [ ] 피드백 수집 및 이슈 트리아지 체계 운영
- [ ] 베타 결과 기반 개선 항목을 다음 스프린트 계획에 반영
